    ...
    <div class="row">
        ...
        <div style="width: 80%">
            <div class="row" style="margin: 10px; justifyContent: center;">
                ...
                <SInput label-text="Location" style="width: 80px; marginRight: 10px;"/>
                <SInput label-text="Floar" style="width: 80px; marginRight: 10px;"/>
            ...
            <div class="grid" :style="bg">
              ...
              >
                <div class="grid2" :style="{backgroundColor: i.color}">
                  <div>{{i.title}}</div>
                  <q-separator/>
                </div>
              </VueDragResize>
        ...
        <DialogFoorPlan @onSave="onSave" :dialogFoorPlan="dialogFoorPlan"/>
    </div>
</template>
...
export default defineComponent({
    setup(_,{root}){
        ...
        const onBack = () => {
          store.commit.layout.SET_STEP(2)
          store.commit.layout.SET_VALIDASI(2)
          store.commit.layout.SET_ROOM(6)
          root.$router.push('/')
        }
        ...
        const onSave = (data) => {
          if (data !== undefined) {
            state.dialogFoorPlan.dialog = false
              pageList.push({
                y: 310,
                x: 100,
                color: '#eb0c0c',
            ...
            NotifyCreate('Select a room not found')
          }
        }
